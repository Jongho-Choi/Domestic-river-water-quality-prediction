...
def load_csv():
    df = pd.read_csv(CSV_FILEPATH, names=['년도','월','회차','수질측정망_명','수질측정망_코드','검사일자','항목코드'
                                      ,'항목명','값','항목정제여부','위도','경도','cat_id','cat_did'], encoding='cp949')
    return df


def EDA(df):
    df = df.pivot(index=['년도','월','회차','검사일자','수질측정망_코드','수질측정망_명','위도','경도'], columns='항목명', values='값')
